...
class ActorCritic:
	...
	def create_actor_model(self):
		...
		action_size = 1
		output = Dense(action_size, activation='tanh')(h3)
	...
	def create_critic_model(self):
		...
		output = Dense(1, activation='tanh')(merged_h1)
	...
	def _train_critic(self, samples):
		for sample in samples:
			...
			self.critic_model.fit([cur_state, np.reshape(action,(1,1))], np.reshape(reward,(1,1)), verbose=0)

	def train(self):
		batch_size = 32
		if len(self.memory) < batch_size:
			print("Memory is smaller than batch_size")
			return
