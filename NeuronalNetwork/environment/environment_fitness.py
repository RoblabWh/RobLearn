...
class FitnessData:
    ...
    def calculate_reward(self, robot_x: float, robot_y: float, robot_orientation: float, env_done: bool):
        ...
        diff_rotation_to_end_last = self.angle_difference_from_robot_to_end(self._robot_x_last, self._robot_y_last,
                                                                            self._robot_orientation_last)
        diff_rotation_to_end = self.angle_difference_from_robot_to_end(robot_x, robot_y, robot_orientation)

        diff_rotations = (math.pi - math.fabs(
            math.fabs(diff_rotation_to_end) - math.fabs(diff_rotation_to_end_last))) / math.pi  # [0 - 1]
        ...
        if distance_robot_to_end > sqrt(distance_between_last_step ** 2 + distance_robot_to_end_last ** 2):
            distance_robot_to_end_diff_abs *= -2  # [-2,0]
        else:
